...
comment-token = ""
# The LSP executable. It should be either an absolute path, or the name of the executable if it is in $PATH. For it to work,
# the LSP must be using stdio. You can also pass arguments to the LSP through the args array.
...
The compiled grammar file will be located in the _project's_ `runtime/grammars` directory (not the one in your config folder).
...
* Update (Nov. 3rd, 2021): The move towards beginning implementation was decided a month ago, but time constraints have delayed it. Please be patient with us :)
